    ...
    <div class="container mx-auto px-4">
      ...
      <div class="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p class="text-gray-600">Administra usuarios, permisos y roles de la plataforma</p>
        </div>
      ...
      >
        <div class="mb-4 rounded-full bg-gray-50 p-4">
          <Icon name="heroicons:users" class="h-10 w-10 text-gray-400" />
        </div>
        <h3 class="text-lg font-medium text-gray-800">No hay usuarios</h3>
        <p class="mt-1 text-gray-500">No se encontraron resultados con los filtros actuales.</p>
        <button
      ...
      <div v-else class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        <div
          ...
          :key="user.id"
          class="group relative flex flex-col rounded-2xl border border-gray-100 bg-white p-5 shadow-sm transition-all hover:-translate-y-1 hover:shadow-md"
        >
          <!-- User Header -->
          <div class="mb-4 flex items-center gap-4">
            <div class="relative h-14 w-14 flex-shrink-0">
              ...
              <div
                v-else
                class="flex h-14 w-14 items-center justify-center rounded-full bg-gray-100 text-lg font-bold text-gray-500"
              >
          ...
          <!-- Actions -->
          <div class="mt-4 flex gap-2 border-t border-gray-50 pt-4">
            <button
              @click="editUser(user)"
              class="flex-1 rounded-lg bg-gray-50 py-2 text-sm font-medium text-gray-600 transition-colors hover:bg-gray-100 hover:text-blue-600"
            >
              Editar
            </button>
          </div>
      ...
      <div v-if="filteredUsers.length > 0" class="mt-8 flex justify-center">
        <nav class="isolate inline-flex -space-x-px rounded-md shadow-sm">
          ...
          <button
            ...
            :class="[
              'relative inline-flex items-center rounded-r-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50',
      ...
      >
        <div
          class="h-full w-full max-w-md overflow-y-auto bg-white p-6 shadow-2xl transition-transform sm:border-l sm:border-gray-100"
        >
          ...
          <div class="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                type="text"
                class="w-full rounded-xl border-gray-200 bg-gray-50 px-4 py-3 font-medium transition-all focus:border-emerald-500 focus:bg-white focus:ring-emerald-500"
                placeholder="Ej. Juan Pérez"
              />
            </div>

            <div>
              <label class="mb-1 block text-sm font-bold text-gray-700">Correo Electrónico</label>
              <input
                v-model="userForm.email"
                type="email"
                class="w-full rounded-xl border-gray-200 bg-gray-50 px-4 py-3 font-medium transition-all focus:border-emerald-500 focus:bg-white focus:ring-emerald-500 disabled:opacity-60"
            ...
            <div v-if="!selectedUser">
              <label class="mb-1 block text-sm font-bold text-gray-700">Contraseña</label>
              <input
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="mb-1 block text-sm font-bold text-gray-700">Rol</label>
                <select
                  v-model="userForm.role"
                  class="w-full rounded-xl border-gray-200 bg-gray-50 px-3 py-3 text-sm font-medium transition-all focus:border-emerald-500 focus:bg-white focus:ring-emerald-500"
                >
                  <option value="user">Usuario</option>
                  <option value="volunteer">Voluntario</option>
                  <option value="shelter">Refugio</option>
            ...
            <div>
              <label class="mb-1 block text-sm font-bold text-gray-700">Teléfono</label>
              <input
            ...
            <div>
              <label class="mb-1 block text-sm font-bold text-gray-700">Dirección</label>
              <textarea
          ...
          <div class="mt-8 flex justify-end space-x-3 border-t border-gray-100 pt-6">
            <button
              class="rounded-xl border border-gray-200 px-6 py-3 text-sm font-bold text-gray-600 transition-colors hover:bg-gray-50"
              @click="closeEditModal"
            >
              Cancelar
            </button>
            <button
              class="rounded-xl bg-emerald-600 px-8 py-3 text-sm font-bold text-white shadow-md transition-colors hover:bg-emerald-700"
              @click="saveUser"
            >
              Guardar usuario
            </button>
...
const paginationEnd = computed(() =>
  Math.min(paginationStart.value + pageSize, filteredUsers.value.length)
)
const paginatedUsers = computed(() =>
  filteredUsers.value.slice(paginationStart.value, paginationEnd.value)
)
