    ...
    <div class="container mx-auto px-4">
      <div class="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p class="text-gray-600">Administra el inventario de adopciones</p>
        </div>
      ...
      >
        ...
        <h3 class="text-lg font-medium text-gray-800">No hay mascotas</h3>
      ...
      <div v-else class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        <div
          ...
          :key="pet.id"
          class="group relative flex flex-col overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm transition-all hover:-translate-y-1 hover:shadow-md"
        >
          <!-- Image Header -->
          <div class="relative h-56 bg-gray-100">
            ...
            <div
              class="absolute bottom-0 left-0 right-0 flex items-center justify-between bg-gradient-to-t from-black/80 via-black/40 to-transparent p-4 pt-12"
            >
              <div class="text-white">
                <h3 class="text-lg font-bold leading-tight">{{ pet.name }}</h3>
                <p class="text-xs capitalize opacity-90">
          ...
          <div class="flex-1 p-4">
            ...
            <div class="mb-4">
              ...
              <div class="relative">
                <select
                  v-model="pet.status"
                  class="w-full appearance-none rounded-lg border-gray-200 bg-gray-50 py-2 pl-3 pr-8 text-sm font-medium text-gray-700 focus:border-emerald-500 focus:ring-emerald-500"
            ...
            <div
              class="flex items-center justify-between rounded-lg border border-gray-100 bg-white p-3"
            >
              <div class="flex items-center gap-2">
                <Icon
                  name="mdi:alert-decagram"
                  :class="pet.urgent ? 'text-amber-500' : 'text-gray-300'"
                  class="h-5 w-5"
                />
                <span class="text-sm font-medium text-gray-700">Caso Urgente</span>
              </div>
              <!-- Urgent Toggle -->
              <button
                ...
                :class="[
                  ...
                  pet.urgent ? 'bg-amber-500' : 'bg-gray-200',
                ]"
              >
                <span
                  aria-hidden="true"
          ...
          <!-- Footer Actions -->
          <div
            class="grid grid-cols-3 divide-x divide-slate-100 border-t border-gray-100 bg-gray-50"
          >
            <button
              @click="viewPet(pet.id)"
              class="flex items-center justify-center py-3 text-gray-500 transition-colors hover:bg-white hover:text-emerald-600"
              title="Ver ficha"
            >
            ...
            <button
              ...
              class="flex items-center justify-center py-3 text-gray-500 transition-colors hover:bg-white hover:text-blue-600"
              title="Editar"
            >
            ...
            <button
              ...
              class="flex items-center justify-center py-3 text-gray-500 transition-colors hover:bg-white hover:text-red-600"
              title="Eliminar"
            >
...
const filteredPets = computed(() => {
  ...
  if (filters.value.type) {
    result = result.filter(
      (pet) => pet.typeValue === filters.value.type || pet.type === filters.value.type
    )
  }
...
async function loadPets() {
  try {
    allPets.value = await fetchAllPetsRaw()
  } catch (err) {
