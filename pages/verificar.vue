  ...
  <div class="min-h-screen bg-amber-50 py-12">
    <div class="container mx-auto px-4">
      <div class="mx-auto max-w-2xl rounded-lg bg-white p-6 shadow">
        <h1 class="mb-4 text-2xl font-bold text-emerald-800">Verificar Certificado</h1>

        <div v-if="loading" class="py-8 text-center">
          <div
            class="mx-auto h-10 w-10 animate-spin rounded-full border-4 border-solid border-emerald-600 border-r-transparent"
          />
          <p class="mt-2 text-gray-600">Buscando verificación...</p>
        </div>

        <div v-else-if="!verification" class="py-8 text-center">
          <Icon name="heroicons:exclamation-triangle" class="mx-auto h-12 w-12 text-yellow-500" />
          <h2 class="mt-4 text-lg font-semibold text-gray-800">Verificación no encontrada</h2>
          <p class="mt-2 text-sm text-gray-600">
            El identificador proporcionado no corresponde a una verificación válida.
          </p>
        </div>
          ...
          <div class="rounded-lg border bg-white p-4">
            ...
            <div class="mt-2">
              <p class="font-medium text-gray-900">
                {{ adopter?.displayName || adopter?.email || 'No disponible' }}
              </p>
